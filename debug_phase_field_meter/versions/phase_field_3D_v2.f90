...
subroutine kjacobian(jelem,ndim,nnode,coords,B_deriv_local,djac,dNdx,mcrd)
...
!     dNdx - shape functions derivatives w.r.t. global coordinates
      ...
      dimension xjac(ndim,ndim),xjaci(ndim,ndim),coords(mcrd,nnode), &
        B_deriv_local(ndim,nnode),dNdx(ndim,nnode)
      ...
      dNdx=matmul(xjaci,B_deriv_local)
...
!     Notation, strain tensor: e11, e22, e33, e12, e13, e23
      ...
      b=0.d0
      do inode=1,nnode
       b(1,ndim*inode-ndim+1)=dNdx(1,inode)
       b(2,ndim*inode-ndim+2)=dNdx(2,inode)
       b(4,ndim*inode-ndim+1)=dNdx(2,inode)
       b(4,ndim*inode-ndim+2)=dNdx(1,inode)
       if (ndim==3) then
        b(3,ndim*inode)=dNdx(3,inode)
        b(5,ndim*inode-2)=dNdx(3,inode)
        b(5,ndim*inode)=dNdx(1,inode)
        b(6,ndim*inode-1)=dNdx(3,inode)
        b(6,ndim*inode)=dNdx(2,inode)
       endif
      end do
...
!   compute the hydrostatic stress
    ...
    do inode=1,nnode
        ...
        do i=1,ninpt
            isvinc=(i-1)*nsvint
            SHa(inode,1)=SHa(inode,1)+dNS(i)*svars(isvinc+14)
        end do
    end do      
...
!   evaluate shape functions and derivatives
        ...
        call kjacobian(jelem,ndim,nnode,coords,B_deriv_local,djac,dNdx,mcrd)
        ! subroutine kjacobian(jelem,ndim,nnode,coords,B_deriv_local,djac,dNdx,mcrd)
...
!   call umat to obtain stresses and constitutive matrix 
        call UMAT_elastic(props,ddsdde,stress,dstran,ntens,statev_local)
...
! output
        user_vars(jelem,1:6,kintk)=statev_local(1:6)*((1.d0-phin)**2+xkap)
        user_vars(jelem,7:14,kintk)=statev_local((ntens+1):(2*ntens+2))

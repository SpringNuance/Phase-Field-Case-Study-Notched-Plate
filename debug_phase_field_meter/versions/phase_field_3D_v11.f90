...
subroutine kshapefcn(kintk,ninpt,nnode,ndim,deriv_N_node_to_int,B_deriv_local)
!   
    use iso_module
    include 'aba_param.inc'
!
      dimension deriv_N_node_to_int(1, nnode),B_deriv_local(ndim,nnode)
...
!     shape functions
    deriv_N_node_to_int(1,1)=0.125d0*(1.d0-f)*(1.d0-g)*(1.d0-h)
    deriv_N_node_to_int(1,2)=0.125d0*(1.d0+f)*(1.d0-g)*(1.d0-h)
    deriv_N_node_to_int(1,3)=0.125d0*(1.d0+f)*(1.d0+g)*(1.d0-h)
    deriv_N_node_to_int(1,4)=0.125d0*(1.d0-f)*(1.d0+g)*(1.d0-h)
    deriv_N_node_to_int(1,5)=0.125d0*(1.d0-f)*(1.d0-g)*(1.d0+h)
    deriv_N_node_to_int(1,6)=0.125d0*(1.d0+f)*(1.d0-g)*(1.d0+h)
    deriv_N_node_to_int(1,7)=0.125d0*(1.d0+f)*(1.d0+g)*(1.d0+h)
    deriv_N_node_to_int(1,8)=0.125d0*(1.d0-f)*(1.d0+g)*(1.d0+h)
...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    real(kind=dp), dimension(1,nnode) :: deriv_N_node_to_int       ! Shape function that interpolates from nodal points to integration points
                                                                    ! The extra 1 dimension is for matrix multiplication, otherwise it would be a vector
    real(kind=dp), dimension(ninpt) :: deriv_N_int_to_node        ! Shape function that extrapolates from integration points to nodal points
...
!   compute the hydrostatic stress
    ...
    do inode=1,nnode
        ...
        deriv_N_int_to_node(1) = 0.125d0 * (1.d0 - xi_node) * (1.d0 - eta_node) * (1.d0 - zeta_node)
        deriv_N_int_to_node(2) = 0.125d0 * (1.d0 + xi_node) * (1.d0 - eta_node) * (1.d0 - zeta_node)
        deriv_N_int_to_node(3) = 0.125d0 * (1.d0 - xi_node) * (1.d0 + eta_node) * (1.d0 - zeta_node)
        deriv_N_int_to_node(4) = 0.125d0 * (1.d0 + xi_node) * (1.d0 + eta_node) * (1.d0 - zeta_node)
        deriv_N_int_to_node(5) = 0.125d0 * (1.d0 - xi_node) * (1.d0 - eta_node) * (1.d0 + zeta_node)
        deriv_N_int_to_node(6) = 0.125d0 * (1.d0 + xi_node) * (1.d0 - eta_node) * (1.d0 + zeta_node)
        deriv_N_int_to_node(7) = 0.125d0 * (1.d0 - xi_node) * (1.d0 + eta_node) * (1.d0 + zeta_node)
        deriv_N_int_to_node(8) = 0.125d0 * (1.d0 + xi_node) * (1.d0 + eta_node) * (1.d0 + zeta_node)
        
        ! print *, 'deriv_N_int_to_node', deriv_N_int_to_node
        do i=1,ninpt
            isvinc=(i-1)*nsvint
            sig_H(inode,1)=sig_H(inode,1)+deriv_N_int_to_node(i)*svars(isvinc+14)
        end do
    end do      
      
    do kintk=1,ninpt
!   evaluate shape functions and derivatives
        call kshapefcn(kintk,ninpt,nnode,ndim,deriv_N_node_to_int,B_deriv_local)      
        ! kshapefcn(kintk,ninpt,nnode,ndim,deriv_N_node_to_int,B_deriv_local)
...
!   compute from nodal values
        ...
        CL_wtppm=0.d0
        do inode=1,nnode
            phi=phi+deriv_N_node_to_int(1,inode)*phi_prev(inode)
            CL_wtppm=CL_wtppm+deriv_N_node_to_int(1,inode)*CL_prev(inode)
        end do   
...
!   enforcing Karush-Kuhn-Tucker conditions
        ...
        amatrx(start_phi_idx:end_phi_idx,start_phi_idx:end_phi_idx) = &
            ...
            + matmul(transpose(deriv_N_node_to_int),deriv_N_node_to_int)*(Gc/xlc+2.d0*history))   
        ...
        rhs(start_phi_idx:end_phi_idx,1) = rhs(start_phi_idx:end_phi_idx,1) &
            ...
            *Gc*xlc+deriv_N_node_to_int(1,:)*((Gc/xlc+2.d0*history)*phi-2.d0*history))
        ...
        M_conc_capacity = matmul(transpose(deriv_N_node_to_int),deriv_N_node_to_int)/DL
        ...
        K_diffusitivity = matmul(transpose(B_deriv_global),B_deriv_global) &
            - VH/(R*T)*matmul(BB,matmul((sig_H * softened_factor),deriv_N_node_to_int))

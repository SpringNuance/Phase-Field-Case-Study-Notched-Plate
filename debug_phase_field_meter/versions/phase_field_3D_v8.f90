...
!   enforcing Karush-Kuhn-Tucker conditions
        ...
        amatrx(start_u_idx:end_u_idx,start_u_idx:end_u_idx) = &
            amatrx(start_u_idx:end_u_idx,start_u_idx:end_u_idx)+dvol*(((1.d0-phin)**2+xkap) &
                                * matmul(matmul(transpose(Bu_matrix),ddsdde),Bu_matrix))
            
        rhs(start_u_idx:end_u_idx,1)=rhs(start_u_idx:end_u_idx,1) - &
            dvol*(matmul(transpose(Bu_matrix),stress)*((1.d0-phin)**2+xkap))       
...
! output
        user_vars(jelem,1:6,kintk)=statev_element(1:6)*((1.d0-phin)**2+xkap)
        user_vars(jelem,7:14,kintk)=statev_element((ntens+1):(2*ntens+2))

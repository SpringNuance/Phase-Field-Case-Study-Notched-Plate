...
!     State variables  
    ...
    ! 7 to 12: eelas11, eelas22, eelas33, eelas12, eelas13, eelas23
    ! 13 to 18: eplas11, eplas22, eplas33, eplas12, eplas13, eplas23
    ...
    ! "7, AR7_eelas11, AR7_eelas11   ",
    ! "8, AR8_eelas22, AR8_eelas22   ",
    ! "9, AR9_eelas33, AR9_eelas33   ",
    ! "10, AR10_eelas12, AR10_eelas12   ",
    ! "11, AR11_eelas13, AR11_eelas13   ",
    ! "12, AR12_eelas23, AR12_eelas23,   ",

    ! "13, AR13_eplas11, AR13_eplas11   ",
    ! "14, AR14_eplas22, AR14_eplas22   ",
    ! "15, AR15_eplas33, AR15_eplas33   ",
    ! "16, AR16_eplas12, AR16_eplas12   ",
    ! "17, AR17_eplas13, AR17_eplas13   ",
    ! "18, AR18_eplas23, AR18_eplas23   ",
    
    ! "19, AR13_eqplas, AR13_eqplas   ", 
    ! "20, AR14_deqplas, AR14_deqplas   ",

    ! "21, AR14_sig_H, AR14_sig_H   ",
    ! "22, AR15_sig_vonMises, AR15_sig_vonMises   ",

    ! "23, AR16_triax, AR16_triax   ",
    ! "24, AR17_lode, AR17_lode   ",

    ! "25, AR18_phi, AR18_phi   ",
    ! "26, AR19_history, AR19_history   ",
    ! "27, AR20_Gc, AR20_Gc   ",
    
    ! "28, AR22_C_mol, AR22_C_mol   ",	
    ! "29, AR23_CL_mol, AR23_CL_mol   ", 
    ! "30, AR24_CT_mol, AR24_CT_mol   ", 
...
module common_block
    ...
    integer, parameter :: eqplas_idx = 19 ! Index of the equivalent plastic strain in statev
    integer, parameter :: deqplas_idx = 20 ! Index of the increment of the equivalent plastic strain in statev
    integer, parameter :: sig_H_idx = 21 ! Index of the hydrogen concentration in statev
    integer, parameter :: sig_vonMises_idx = 22 ! Index of the equivalent von Mises stress in statev
    ...
    integer, parameter :: phi_idx = 25 ! Index of the phase field parameter in statev
    integer, parameter :: history_idx = 26 ! Index of the history variable in statev
    integer, parameter :: Gc_idx = 27 ! Index of the fracture energy in statev
    ! integer, parameter :: theta_coverage_idx = 21 ! Index of the hydrogen coverage in statev
    integer, parameter :: C_mol_idx = 28 ! Index of the hydrogen concentration in the matrix in statev
    integer, parameter :: CL_mol_idx = 29 ! Index of the hydrogen concentration in the lattice in statev
    integer, parameter :: CT_mol_idx = 30 ! Index of the hydrogen concentration in the trap in statev
    ...
    real(kind=dp) :: sig_H_all_elem_at_nodes(100000, 8) ! (nelem, nnode)
...
!   compute the hydrostatic stress
    ...
    do inode = 1,nnode
        ...
        sig_H_node(inode,1) = total_sig_H_current_node / dble(count_elem)
    end do      
...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    real(kind=dp) :: E, nu, lambda, mu, kappa, hard_mod, syield
...
!   reading parameters
    ...
    do kintk=1,ninpt
        ...
        eelas = statev_element((ntens+1):(2*ntens))
        eplas = statev_element((2*ntens+1):(3*ntens))
        stran = eelas + eplas
        history_n = statev_element(history_idx)
       
        !   Yin et al
        
        ! First invariant of the elastic strain tensor
        eelas_I1 = eelas(1) + eelas(2) + eelas(3)

        ! Selective operator
        selective_eelas_I1_plus = half * (eelas_I1 + abs(eelas_I1))
        selective_eelas_I1_minus = half * (eelas_I1 - abs(eelas_I1))
        
        dev_eelas11 = eelas(1) - third * selective_eelas_I1_plus
        dev_eelas22 = eelas(2) - third * selective_eelas_I1_plus
        dev_eelas33 = eelas(3) - third * selective_eelas_I1_plus
        dev_eelas12 = eelas(4)
        dev_eelas13 = eelas(5)
        dev_eelas23 = eelas(6)
        dev_eelas_total = dev_eelas11 * dev_eelas11 + &
                          dev_eelas22 * dev_eelas22 + &
                          dev_eelas33 * dev_eelas33 + &
                  2.0d0 * dev_eelas12 * dev_eelas12 + &
                  2.0d0 * dev_eelas13 * dev_eelas13 + &
                  2.0d0 * dev_eelas23 * dev_eelas23
        ...
        lambda = E * nu/((1.0d0 + nu) * (1.0d0 - 2.0d0 * nu)) ! Lame's first constant
        ! Bulk modulus
        kappa = lambda + two_third * mu


        psi_elas_plus = half * kappa * selective_eelas_I1_plus * selective_eelas_I1_plus + &
                        mu * dev_eelas_total
        
        ! if (kinc == 2) then
        !     print *, "eelas_I1 = ", eelas_I1
        !     print *, "selective_eelas_I1_plus = ", selective_eelas_I1_plus
        !     print *, "selective_eelas_I1_minus = ", selective_eelas_I1_minus
        !     print *, "dev_eelas11 = ", dev_eelas11
        !     print *, "dev_eelas22 = ", dev_eelas22
        !     print *, "dev_eelas33 = ", dev_eelas33
        !     print *, "dev_eelas12 = ", dev_eelas12
        !     print *, "dev_eelas13 = ", dev_eelas13
        !     print *, "dev_eelas23 = ", dev_eelas23
        !     print *, "dev_eelas_total = ", dev_eelas_total
        !     print *, "mu = ", mu
        !     print *, "lambda = ", lambda
        !     print *, "kappa = ", kappa
        !     print *, "psi_elas_plus = ", psi_elas_plus
        ! end if
        
        ! psi_elas_minus = half * kappa * selective_eelas_I1_minus * selective_eelas_I1_minus 

        ! psi_plas = syield * eqplas + half * hard_mod * eqplas * eqplas

        !   compute elastic strain energy density from the previous increment       

        !   enforcing Karush-Kuhn-Tucker conditions
        if (psi_elas_plus > history_n) then
            history = psi_elas_plus
        else
            history = history_n
        endif
        ...
        ! Calculate ddsdde_elas

        ! mu = E/(2.0d0 * (1.0d0 + nu))  ! Shear modulus
        ! lambda = E * nu/((1.0d0 + nu) * (1.0d0 - 2.0d0 * nu)) ! Lame's first constant

        ! ! initialize as 0
        ! ddsdde_elas = 0.0 ! Their unit is Pa
        
        ! do i = 1, ndi
        !     do j = 1, ndi
        !         ddsdde_elas(j, i) = lambda
        !     end do 
        !     ddsdde_elas(i,i) = lambda + 2.0 * mu
        ! end do 
        ...
        ! ! Initialize psi_elas to 0.0
        ! psi_elas = 0.0d0

        ! ! Initialize temporary array to 0.0
        ! temp = 0.0d0

        ! ! Step 1: Calculate the intermediate result (ddsdde_elas * eelas)
        ! ! temp(i) = sum over j of ddsdde_elas(i, j) * eelas(j)
        ! do i = 1, 6
        !     do j = 1, 6
        !         temp(i) = temp(i) + ddsdde_elas(i, j) * eelas(j)
        !     end do
        ! end do

        ! ! Step 2: Calculate psi_elas as (1/2) * eelas^T * temp
        ! ! psi_elas = (1/2) * sum over i of eelas(i) * temp(i)
        ! do i = 1, 6
        !     psi_elas = psi_elas + eelas(i) * temp(i)
        ! end do

        ! ! Multiply by 1/2 to get the final elastic energy density
        ! psi_elas = half * psi_elas
        
        ! psi_elas = 0.0d0
        ! do k1 = 1,ntens
        !     psi_elas = psi_elas + stress(k1) * eelas(k1) * 0.5d0
        !     ! psi_elas = psi_elas + stress(k1) * stran(k1) * 0.5d0
        ! end do

        ! !   enforcing Karush-Kuhn-Tucker conditions
        ! if (psi_elas > history_n) then
        !     history = psi_elas
        ! else
        !     history = history_n
        ! endif
        ...
        statev_element((ntens+1):(2*ntens)) = eelas(1:ntens)
        statev_element((2*ntens+1):(3*ntens)) = eplas(1:ntens)
        ...
        user_vars(jelem,ntens*3+1:nsvint,kintk) = statev_element(ntens*3+1:nsvint)

...
inp_file_name = 'phase_field_plate_3D'
...
description_properties_dict = {
    "mechanical_properties": {
        "Young's modulus E (Pa)": "210e9", # correct
        "Poisson ratio nu (dimless)": "0.3", # correct
        "density rho (kg/m^3)": "7900.0", # correct
    },
    # 0.05, 2.7, 0.0127, Gibbs free 3.0d7 N.mm/mol   
    # Gc(0) = 2.7 MPa / mm = 2700 Pa / m = 2700 J/m^2
    # Note: J = N*m
    "phase_field_damage_properties": {
        "length scale lc (m)": "5.0e-5", # correct
        "critical energy release rate Gc (N*m/m^2)": "2700e0", # correct
        ...
        "Gibbs free energy difference between the decohering interface and the surrounding material delta_g_b0 (N*m/mol)": "30000.0e0", # correct
    },
    # R = 8314.5 N.mm/(mol.K), T = 300 K, VH = 2000 mm^3/mol, DL = 0.0127 mm^2/s
    "hydrogen_diffusion_properties": {
        "universal gas constant R (N*m)/(mol*K)": "8.3145e0", # correct
        "temperature T (K)": "300e0", # correct
        "partial molar volume VH (m^3/mol)": "2e-6", # correct
        "diffusion coefficient DL (m^2/s)": "1.27e-8", # correct
        "trap mode": "3", # dadfarnia
        "crystal structure": "1", # BCC
        "Avogadro constant (1/mol)": "6.022e23",
        "number of solvent atoms NL (1/m^3)": "8.518e28",
        ...
        "number of interstitial sites per solvent atom - beta_BCC (dimless)": "6.0",
        "number of interstitial sites per solvent atom - beta_FCC (dimless)": "1.0",
        "lattice parameter a_lattice_BCC (m)": "2.866e-10",
        "lattice parameter a_lattice_FCC (m)": "3.571e-10",
        "gamma fitting parameter (1/m^2)": "2.0e16",
        "dislocation density for annealed material rho_d0 (1/m^2)": "1.0e10"
    },
...
def return_depvar(nstatev):
    DEPVAR = [
        ...
        "19, AR19_eqplas, AR19_eqplas   ", 
        "20, AR20_deqplas, AR20_deqplas   ",

        "21, AR21_sig_H, AR21_sig_H   ",
        "22, AR22_sig_vonMises, AR22_sig_vonMises   ",

        "23, AR23_triax, AR23_triax   ",
        "24, AR24_lode, AR24_lode   ",

        "25, AR25_phi, AR25_phi   ",
        "26, AR26_history, AR26_history   ",
        "27, AR27_Gc, AR27_Gc   ",

        "28, AR28_C_mol, AR28_C_mol   ",	
        "29, AR29_CL_mol, AR29_CL_mol   ", 
        "30, AR30_CT_mol, AR30_CT_mol   ", 
    ]
...
def constructing_visualization_mesh(input_file_path):
    ...
    for i in range(num_elements):
        ...
        original_line = " ".join([str(value) for value in [element_indices[i]] + element_connvectivity[i]])
        original_mesh.append(original_line)
        visualization_mesh.append(reconstructed_line)
    
    return visualization_mesh, original_mesh, start_element_index, end_element_index

VISUALIZATION_MESH, original_mesh, start_element_index, end_element_index =\
    constructing_visualization_mesh(combined_original_inp_path)

# Save the original mesh as .txt file

with open(f'{output_simulation_path}/original_mesh.txt', 'w') as fid:
    for i, line in enumerate(original_mesh):
        if i == len(original_mesh) - 1:
            fid.write(line)
        else:
            fid.write(line + "\n")

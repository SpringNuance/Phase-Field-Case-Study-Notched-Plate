! -----------------------------------------------------------C
! Subroutine for Abaqus/Explicit for isotropic elasticity    C
! and isotropic plasticity with pressure and lode            C
! dependence, according to plasticity model by BAI, Y. and   C
! WIERZBICKI, T. (published in "A new model of metal         C
! plasticity and fracture with pressure and Lode             C
! dependence", International Journal of Plasticity 24 (2008) C
! pages 1071-1096).                                          C
!															 C
! - Damage initiation according to 3D fracture locus         C
!   (equivalent plastic strain to failure initiation being   C
!   function of both hydrostatic pressure and lode angle)    C
! - Damage evolution based on energy dissipated during       C
!   damage process                                           C
!															 C
! - Temperature softening									 C
! - Strain rate hardening									 C
!                                                            C
! Originally based on the von Mises' J2 plasticity theory.   C
! Elastic predictor, radial corrector algorithm used. With   C
! explicit forward Euler scheme for integration of flow rule.C
!                                                            C
! Not suitable for solving problem settings including plane  C
! stresses. Not suitable for 2D models.                      C
! -----------------------------------------------------------C
! Solution dependent variables (SDVs):						 C
C															 C
! SDV1: 	Equivalent plastic strain						 C
! SDV2: 	Damage variable									 C
! SDV3: 	Yield stress at damage initiation				 C
! SDV4: 	Flag (=0 element not damaged,					 C
C			      =1 element experienced brittle fracture,	 C
C			      =2 element experienced ductile damage,     C
!                 =3 element experienced ductile fracture)	 C
! SDV5-10:	Total strain tensor								 C
! SDV11-16:	Plastic strain tensor							 C
! SDV17:	Equivalent plastic strain increment				 C
! SDV18:	Temperature softening correction function		 C
! SDV19:	Mutliplicative strain rate hardening correction	 C
C			function		 								 C
! SDV20:	Equivalent plastic strain rate					 C
! SDV21:	Equivalent plastic strain rate (computed over	 C
!           a user-defined no. of increments)				 C
! SDV22:	Increment counter (for eq. plas. str. rate       C
!           computation)									 C
! SDV23:	Sum of equivalent plastic strain increments      C
!           before the computation of eq. plas. str. rate    C
! SDV24:	Sum of time increments before computation of eq. C
!           plas. str. rate									 C
! SDV25:	Additive strain rate hardening correction		 C
C			function										 C
! SDV26:	Maximum principal stress						 C
! SDV27:	Equivalent plastic strain (averaged over a		 C
C			user-defined no. of increments)					 C
! SDV28:	Element deletion flag							 C
! -----------------------------------------------------------C
! Contact info:                                              C
C															 C
! Mohamed Sharaf                                             C
! RWTH Aachen                                                C
! Institut fuer Eisenhuettenkunde                            C
! Intzestrasse 1                                             C
! 52072 Aachen                                               C
! Germany                                                    C
! mohamed.sharaf@iehk.rwth-aachen.de                         C
!                                                            C
! Aachen, 6 March 2012                                       C
C------------------------------------------------------------C
! SDV and parameters modified by B. Wu, J. He and F. Shen    C
!                                                            C
! SDV29:	Eta         								     C
! SDV30:	Thetabar										 C
! SDV31:	Ductile damage initiation indicator				 C
! SDV32:	Ductile failure indicator						 C
! SDV33:	Damage initiation strain at the current step	 C
! SDV34:	Dcrit at the current step						 C
! SDV35:	Cleavage initiation strain at the current step	 C
! SDV36:	Cleavage failure indicator based on strain		 C
! SDV37:	Damage fracture strain at the current step	     C
! SDV38:	Equivalent plastic strain at damage initiation	 C
! -----------------------------------------------------------C
! Additional parameters props(28)-props(36)                  C
!                                                            C
! Additional parameter for cut-off value is defined			 C
! Additional parameters Ddf1-Ddf4 are defined for Ddf-locus  C
! Additional parameters Cc1-Cc4 are defined for Cleavage 	 C
! -----------------------------------------------------------C
! Modifications by B. Wu, J. He and F. Shen                  C
!                                                            C
! Damage indicator: Damage initiates when SDV31>1            C
! Failure indicator: Failure happens when SDV32>1            C
! Cleavage failure indicator: Cleavage happens when SDV36>1  C
! Non-proportional Loading paths with varying stress states  C
! are considered are considered with weighting scheme        C
! Cut-off value concept is considered                        C
! -----------------------------------------------------------C
...
! Calculating the magnitude of the deviatoric trial stress tensor
     ...
     1   **2 + two*sig6**2 )
C
! Preventing a divide by zero when dsmag is zero. When dsmag is zero, 
! computation is still in the elastic zone
...
C
! Eq. (13) - Determining cthetaax, either tension or compression

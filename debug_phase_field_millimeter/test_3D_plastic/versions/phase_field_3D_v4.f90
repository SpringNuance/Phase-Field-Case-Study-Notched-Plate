...
!   enforcing Karush-Kuhn-Tucker conditions
        ...
        rhs(1:24,1)=rhs(1:24,1) - dvol*(matmul(transpose(Bu_matrix),stress)*((1.d0-phin)**2+xkap))       
        ...
        rhs(25:32,1)=rhs(25:32,1) &
            -dvol*(matmul(transpose(B_deriv_global),matmul(B_deriv_global,u(25:32))) &
            *Gc*xlc+dN(1,:)*((Gc/xlc+2.d0*H)*phi-2.d0*H))
            
        xm=matmul(transpose(dN),dN)/D
        BB=matmul(transpose(B_deriv_global),B_deriv_global)
        xk=BB-Vh/(R*T)*matmul(BB,matmul((sig_H*((1.d0-phin)**2+xkap)),dN))

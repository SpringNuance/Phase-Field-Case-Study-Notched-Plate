...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    real(kind=dp) :: xlc, Gc0, xkap, chi_DFT, delta_g_b0, R, T, VH, DL
...
!   evaluate shape functions and derivatives
        ...
        ! kshapefcn(kintk,ninpt,nnode,ndim,shape_node_to_int,B_deriv_local)
...
!   hydrogen contribution  
        ...
        thetaL = CL_wtppm*5.5d-05/(CL_wtppm*5.5d-05+dexp(-delta_g_b0/(R*T)))
...
!   enforcing Karush-Kuhn-Tucker conditions
        ...
        amatrx(start_phi_idx:end_phi_idx,start_phi_idx:end_phi_idx) = &
            ...
            + matmul(transpose(shape_node_to_int),shape_node_to_int)*(Gc/xlc+2.d0*history))   
        ...
        rhs(start_phi_idx:end_phi_idx,1) = rhs(start_phi_idx:end_phi_idx,1) &
            ...
            *Gc*xlc+shape_node_to_int(1,:)*((Gc/xlc+2.d0*history)*phi-2.d0*history))
        ...
        ! K_diffusitivity = matmul(transpose(B_deriv_global),B_deriv_global) &
        !     - VH/(R*T)* matmul(B_deriv_global, softened_grad_sig_H_int)

...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    dimension dN(1,nnode),dNs(ninpt)
...
!   enforcing Karush-Kuhn-Tucker conditions
        ...
        M_conc_capacity=matmul(transpose(dN),dN)/D
        BB=matmul(transpose(B_deriv_global),B_deriv_global)
        K_diffusitivity=BB-Vh/(R*T)*matmul(BB,matmul((sig_H* softened_factor),dN))
        ...
        rhs(start_CL_idx:end_CL_idx,1)=rhs(start_CL_idx:end_CL_idx,1)- &
            dvol*(matmul(K_diffusitivity,u(start_CL_idx:end_CL_idx))+ &
            matmul(M_conc_capacity,du(start_CL_idx:end_CL_idx,1))/dtime)

...
!          Hydrogen diffusion model: Fick's law
...
module filepath
    character(len=256), parameter :: filename = "C:\LocalUserData\User-data\nguyenb5\debug_phase_field_meter_many_SDV_diffusion\original_mesh.txt"
    integer, parameter :: num_elem = 20596
end module filepath
...
module common_block
    ...
    ! molar_mass_H) / (density_metal * 1.d-03)
    real(kind=dp), parameter :: conversion_mol_to_wtppm = 0.127574683544d0 ! wtppm
    real(kind=dp), parameter :: avogadro = 6.022d23 ! 1/mol
    ...
    ! remember that fortran starts counting from 1
    ! So for example if we want to extract Young's modulus from props
    ! We should use props(start_mech_props_index + 1)

    integer, parameter :: start_mech_props_index = 0 ! Index of the first mechanical property in props
    integer, parameter :: start_phase_props_idx = 8 ! Index of the first phase field property in props
    integer, parameter :: start_hydro_props_idx = 16 ! Index of the first hydrogen diffusion property in props
    integer, parameter :: start_flow_props_idx = 40 ! Index of the first flow curve data in props
    ...
    ! Then connect_matrix(1, 1:8) = (/10, 20, .., 80/)
    ...
    ! Then connect_matrix(2, 1:8) = (/10, 20, -1, -1, -1, -1, -1, -1/)
    integer :: connect_matrix(120000, 8) !
...
subroutine UEXTERNALDB(lop,lrestart,time,dtime,kstep,kinc)
    ...
    if (lop == 0) then 
        ...
        ! Initialize connect_matrix with -1 to indicate unused slots
        connect_matrix = -1    
        ...
        ! Read the file line by line and populate the connect_matrix
        do elem_id = 1, num_elem
            ...
            do i = 2, 9 ! Looping over the 8 nodes
                ...
                do j = 1, 8
                    if (connect_matrix(node_id, j) == -1) then
                        connect_matrix(node_id, j) = element_id
        ...
        !     print *, 'Node', i, ': ', connect_matrix(i, 1:8)
        ! end do
...
subroutine UMAT_elastic(props,nprops,ddsdde,stress,sig_vonMises, &
...
!   Subroutine with the material model
    ...
    E = props(start_mech_props_index+1)           ! Young's modulus 
    nu = props(start_mech_props_index+2)          ! Poisson's ratio 
...
subroutine UMAT_von_Mises(props,nprops,ddsdde,stress,sig_vonMises, &
    ...
    ! ----------------------------------------------------------------
    ! PROPS(start_mech_props_index+1) - E
    ! PROPS(start_mech_props_index+2) - NU
    ! PROPS(start_flow_props_index+1:nprops) - SYIELD AN HARDENING DATA
    ! props(start_flow_props_index+1) - syiel0, 
    ! props(start_flow_props_index+2) - eqpl0, 
    ! props(start_flow_props_index+3) - syiel1, 
    ! props(start_flow_props_index+4) - eqpl1, ...
...
subroutine UMATHT_diffusion(eqplas, deqplas, &
    ...
    R               = props(start_hydro_props_idx+1) ! Universal gas constant (8.31446 J/(mol K))
    T               = props(start_hydro_props_idx+2) ! Temperature (300 K)
    VH              = props(start_hydro_props_idx+3) ! Partial molar volume of hydrogen (2e-06 m^3/mol), 
    DL              = props(start_hydro_props_idx+4) ! Diffusion coefficient for lattice hydrogen in the steel (m^2/s), from e-10 to e-15
                               ! This parameter varies widely for different steels and different conditions
    mode            = props(start_hydro_props_idx+5) ! Mode of the traps
    mode_tol        = 1.0e-3
    ...
    crystal_structure = props(start_hydro_props_idx+6) ! Crystal structure (1 - BCC, 2 - FCC)
    crystal_tol     = 1.0e-3 ! same for above reason

    avogadro        = props(start_hydro_props_idx+7) ! Avogadro's constant (6.022e23 1/mol)
    ...
    NL              = props(start_hydro_props_idx+8) ! Number of solvent metal atoms per unit volume (8.518e28 1/m^3)
    alpha_dis       = props(start_hydro_props_idx+9) ! Number of interstitial sites per trap site (dislocations) (1.0 dimless)
    alpha_gb        = props(start_hydro_props_idx+10) ! Number of interstitial sites per trap site (grain boundaries) (1.0 dimless)
    alpha_carb      = props(start_hydro_props_idx+11) ! Number of interstitial sites per trap site (carbides) (1.0 dimless)
    NT_dis          = props(start_hydro_props_idx+12) ! Number of trap type of dislocations (1/m^3) (0 - instead defined by Dadfarnia et al. 2016)
    NT_gb           = props(start_hydro_props_idx+13) ! Number of trap type of grain boundaries (1/m^3) (8.464e22)
    NT_carb         = props(start_hydro_props_idx+14) ! Number of trap type of carbides (1/m^3) (8.464e26)
    WB_dis          = props(start_hydro_props_idx+15) ! Binding energy of hydrogen to dislocations (- 20.2e3) (J/mol)
    WB_gb           = props(start_hydro_props_idx+16) ! Binding energy of hydrogen to grain boundaries (- 58.6e3) (J/mol)
    WB_carb         = props(start_hydro_props_idx+17) ! Binding energy of hydrogen to carbides (- 11.5e3) (J/mol)
    beta_BCC        = props(start_hydro_props_idx+18) ! Number of number of hydrogen atoms that can reside in each lattice site (6.0 dimless)
    beta_FCC        = props(start_hydro_props_idx+19) ! Number of number of hydrogen atoms that can reside in each lattice site (1.0 dimless)
    ! References for lattice parameter
    a_lattice_BCC   = props(start_hydro_props_idx+20) ! Lattice parameter (2.866 Angstrom or 2.866e-10 m)
    a_lattice_FCC   = props(start_hydro_props_idx+21) ! Lattice parameter (3.571 Angstrom or 3.571e-10 m)
    gamma           = props(start_hydro_props_idx+22) ! gamma fitting parameter in Dadfarnia et al. (2.0e16 1/m^2)
    rho_d0          = props(start_hydro_props_idx+23) ! Dislocation density for the annealed material in Dadfarnia et al. (1.0e10 1/m^2)
...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    real(kind=dp), dimension(nnode) :: CL_mol_prev, dCL_mol_prev
    real(kind=dp), dimension(nnode) :: new_CL_mol_prev, new_dCL_mol_prev
    real(kind=dp), dimension(ninpt) :: C_mol_int 
    ...
    real(kind=dp) :: sig_vonMises, sig_hydrostatic, eqplas, deqplas, triaxility, lode_angle
...
!   find total number of elements and stored it to nelem       
    ...
    CL_mol_prev(1:nnode)        = u(start_CL_idx:end_CL_idx)

    du_prev(1:ndim * nnode) = du(start_u_idx:end_u_idx, 1)
    dphi_prev(1:nnode)      = du(start_phi_idx:end_phi_idx, 1)
    dCL_mol_prev(1:nnode)       = du(start_CL_idx:end_CL_idx, 1)

!   reading parameters
    length_scale=props(start_phase_props_idx+1)
    Gc0 = props(start_phase_props_idx+2) ! Critical energy release rate in the absence of hydrogen
    xkap = props(start_phase_props_idx+3) ! Well-conditioning parameter
    chi_DFT = props(start_phase_props_idx+4) ! Fitting slope to the DFT data
    delta_g_b0 = props(start_phase_props_idx+5) ! Gibbs free energy difference between the decohering interface and the surrounding material
    
    R = props(start_hydro_props_idx+1) ! Universal gas constant R (N*m)/(mol*K))
    T = props(start_hydro_props_idx+2) ! Temperature (K)
    VH = props(start_hydro_props_idx+3) ! Molar volume of H (m^3/mol)
    DL = props(start_hydro_props_idx+4) ! Diffusion coefficient of lattice species hydrogen
...
!   compute the hydrostatic stress
    ...
    !  Calculate hydrostatic stress at nodal points
    ...
    do kintk=1,ninpt
        ...
        ! call UMAT_elastic(props,nprops,ddsdde,stress,sig_vonMises, &
        ...
        CL_mol = 0.d0  ! This value is defined at integration point 
                       ! (current, not previous CL concentration)
        dCL_mol = 0.d0 ! This value is defined at integration point
                       ! This is difference between CL_mol and previous CL_mol

        do inode = 1,nnode
            CL_mol = CL_mol + shape_node_to_int(1,inode) * CL_mol_prev(inode)
            dCL_mol = dCL_mol + shape_node_to_int(1,inode) * dCL_mol_prev(inode)
        end do   

        call UMATHT_diffusion(eqplas, deqplas, &
            C_mol, CL_mol, new_CL_mol, dCL_mol, CT_mol, &
            CT_dis_mol, CT_gb_mol, CT_carb_mol, &
            thetaL, thetaT_dis, thetaT_gb, thetaT_carb, &
        ...
        CL_wtppm = C_mol * conversion_mol_to_wtppm
        ...
        ! Extrapolate the hydrogen concentration from integration points to nodal points
        ...
        rhs(start_CL_idx:end_CL_idx,1) = rhs(start_CL_idx:end_CL_idx,1) - &
            dvol * (matmul(K_diffusitivity,CL_mol_prev)+ &
            matmul(M_conc_capacity,dCL_mol_prev)/dtime)

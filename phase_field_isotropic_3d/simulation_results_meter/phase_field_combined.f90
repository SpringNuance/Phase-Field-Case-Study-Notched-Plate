...
!      4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
    ...
    real(kind=dp), dimension(ndim, nnode) :: softened_grad_sig_H    ! Softened hydrostatic stress gradient at each node
    ...
    ! print *, 'nsvars: ', nsvars ! 160 = 8 integration points x 20 state variables
    ! print *, 'nprops: ', nprops ! 240
    ! print *, 'ndofel: ', ndofel ! 40 = 8 nodes x 5 dof
    ! print *, 'mlvarx: ', mlvarx ! 44
    ! print *, 'mcrd: ', mcrd ! 3
    ! print *, 'nrhs: ', nrhs ! 1
    ! print *, 'njprop: ', njprop ! 0
    ! print *, 'jelem: ', jelem ! 1
...
! Hydrogen diffusion parameters are from 17 to 31
    ...
    do inode=1, nnode
        ...
        ! do kintk = 1, ninpt 
        !     xi_gauss = xi_gauss_extra(kintk)
        !     eta_gauss = eta_gauss_extra(kintk)
        !     zeta_gauss = zeta_gauss_extra(kintk)
            
        !     N_shape_gauss_to_nodal(kintk) = 0.125d0 * (1.d0 + xi_gauss * g) &
        !                                                 * (1.d0 + eta_gauss * h) &
        !                                                 * (1.d0 + zeta_gauss * r)
        ! end do
        ...
        do i = 1, ninpt
            ...
            !if (kinc == 20.d0) then    
                ! print *, 'N_shape_gauss_to_nodal(kintk): ', N_shape_gauss_to_nodal(kintk)        
                ! print *, 'svars(isvinc + sig_H_index): ', svars(isvinc + sig_H_index)
            !end if
        end do
    end do    
    ...
    !   Equation 38
        ...
        softened_sig_H = sig_H * softened_factor
        softened_grad_sig_H = matmul(transpose(B_deriv_global), softened_sig_H) ! shape (ndim, nnode)
        ! Equation 45
        ...
        ! K_diffusitivity = BB - VH/(R*T) * matmul(BB, matmul((sig_H * softened_factor), N_shape_nodal_to_gauss))

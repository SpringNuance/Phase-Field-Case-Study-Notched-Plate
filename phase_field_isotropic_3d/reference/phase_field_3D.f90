...
module visualization
    ...
    ! Second dim: number of statev per integration point
    ! Third dim: number of integration points
    real*8 user_vars(100000,11,4)
    integer nelem
    save
end module
...
!   call umat to obtain stresses and constitutive matrix 
        call UMAT_von_Mises(props,ddsdde,stress,dstran,ntens,statev_local)
        stran=stran+dstran
...
! output
        user_vars(jelem,1:4,kintk)=statev_local(1:4)*((1.d0-phin)**2+xkap)
        user_vars(jelem,5:10,kintk)=statev_local((ntens+1):(2*ntens+2))
...
subroutine UMAT_von_Mises(props,ddsdde,stress,dstran,ntens,ndi,nshr,statev)
!
!   Subroutine with the mechanical model for the von Mises plasticity model
!
    include 'aba_param.inc' !implicit real(a-h o-z)
